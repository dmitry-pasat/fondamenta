	...
	<table cellspacing="0" class="woocommerce-grouped-product-list group_table">
		...
		foreach ( $grouped_products as $grouped_product ) {
			if (is_object($grouped_product)) {
				...
				$post    = $product->post;
				$quantites_required = $quantites_required || ( $product->is_sold_individually() || ! $product->is_purchasable() );
			}
			setup_postdata( $post );
			...
			foreach ( $grouped_product_columns as $column_id ) {
				...
				switch ( $column_id ) {
					case 'quantity':
						...
						if ( ! $grouped_product->is_purchasable() || (method_exists($grouped_product, 'has_options') ? $grouped_product->has_options() : false) || ! $grouped_product->is_in_stock() ) {
							...
							woocommerce_quantity_input( array(
								...
								'max_value' => apply_filters( 'woocommerce_quantity_input_max', ( method_exists( $grouped_product, 'get_max_purchase_quantity' ) ? $grouped_product->get_max_purchase_quantity() : ( $product->backorders_allowed() ? '' : $product->get_stock_quantity() ) ), $grouped_product ),
							) );
					...
					case 'label':
						...
						$value .= $grouped_product->is_visible() ? '<a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink( $grouped_product->get_id() ), $grouped_product->get_id() ) ) . '">' . ( method_exists($grouped_product, 'get_name') ? $grouped_product->get_name() : $grouped_product->get_title() ) . '</a>' : ( method_exists($grouped_product, 'get_name') ? $grouped_product->get_name() : $grouped_product->get_title() );
						$value .= '</label>';
						break;
					case 'price':
						...
						if (function_exists('wc_get_stock_html')) {
							...
							if ( $availability = $product->get_availability() ) {
								...
								$value .= apply_filters( 'woocommerce_stock_html', $availability_html, $availability['availability'], $grouped_product );
							}
						}
						break;
					default:
